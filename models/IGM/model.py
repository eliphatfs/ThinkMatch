...
class Net(nn.Module):
    def __init__(self):
        ...
        self.ot = SinkhornDistance(0.05, 10, 'mean')
    ...
    def forward(self, data_dict, **kwargs):
        ...
        key_mask_src = torch.arange(y_src.shape[-1], device=ns_src.device).expand(len(y_src), y_src.shape[-1]) >= ns_src.unsqueeze(-1)
        key_mask_tgt = torch.arange(y_tgt.shape[-1], device=ns_tgt.device).expand(len(y_tgt), y_tgt.shape[-1]) >= ns_tgt.unsqueeze(-1)
        folding_src[torch.stack([key_mask_src] * 2, -1)] = folding_src.mean(-2, keepdim=True).unsqueeze(-2)
        folding_tgt[torch.stack([key_mask_tgt] * 2, -1)] = folding_tgt.mean(-2, keepdim=True).unsqueeze(-2)
