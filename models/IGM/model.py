...
class Net(nn.Module):
    ...
    def forward(self, data_dict, **kwargs):
        ...
        folding_src = (folding_src - folding_src.min(-2, keepdim=True)) / torch.clamp_min(folding_src.max(-2, keepdim=True) - folding_src.min(-2, keepdim=True), 1e-12)
        folding_tgt = (folding_tgt - folding_tgt.min(-2, keepdim=True)) / torch.clamp_min(folding_tgt.max(-2, keepdim=True) - folding_tgt.min(-2, keepdim=True), 1e-12)
